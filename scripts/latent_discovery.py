...
class StyleGAN2Wrapper(nn.Module):
    ...
    def get_w(self, z):
        ...
        """
        return map_for_stylegan(self.G.mapping(z, None))
    ...
    def forward(self, z, shift=None, latent_is_w=False):
        ...
        if self.shift_in_w_space:
            ...
            if latent_is_w:
                ...
                w = map_for_stylegan(self.G.mapping(z, None))
...
def find_latent_linear(run, G, out_dir, model_folder, logger: logging.Logger):
    ...
    def feed_G(G, input, shift=None, is_latent=shift_in_w_space):
        return G(torch.squeeze(input, (2, 3)), shift, is_latent) if is_style_gan else G(input if shift is None else input+shift)
